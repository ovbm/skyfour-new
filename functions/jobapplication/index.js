...
const API_KEY = 'key-8298081cd3fed30274f6cb1edfd1ede3';
const DOMAIN = 'mg.skyfour.ch';
...
exports.sendMail = (req, res) => {
  ...
  if (req.method === 'OPTIONS') {
    ...
    busboy.on('finish', () => {
      // Process saved files
      // create email for tmrow
      const tmrowmaildata = {
        from: `${fields.email}`,
        to: 'ramon.oliveras@skyfour.ch',
      ...
      const usermaildata = {
        from: 'ramon.oliveras@skyfour.ch',
        to: `${fields.email}`,
      ...
      // send mail to jobs@tmrow.com
      mailgun.messages().send(tmrowmaildata, (err, body) => {
        if (err) {
          // log error
